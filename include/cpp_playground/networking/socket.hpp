...
public:
  ...
  [[nodiscard]] auto set_non_blocking(bool non_blocking = true) const noexcept
      -> ErrorHandling::Result<void, std::string> {
    if (m_socket_fd < 0) {
      return ErrorHandling::Err(
          std::format("{}: cannot modify invalid socket",
                      ErrorHandling::format_source_location()));
    }
    int flags = ::fcntl(m_socket_fd, F_GETFL, 0);
    if (flags == -1) {
      return ErrorHandling::Err(
          std::format("{}: failed to get current socket flags: {}",
                      ErrorHandling::format_source_location(),
                      ErrorHandling::get_error_message(errno)));
    }
    if (non_blocking) {
    ...
    if (::fcntl(m_socket_fd, F_SETFL, flags) == -1) {
      return ErrorHandling::Err(
          std::format("{}: failed to set new socket flags: {}",
                      ErrorHandling::format_source_location(),
                      ErrorHandling::get_error_message(errno)));
    }
    return {};
  ...
  [[nodiscard]] auto bind(const Address &address) const
      -> ErrorHandling::Result<void, std::string> {
    if (::bind(m_socket_fd, (const ::sockaddr *)&address, sizeof(Address)) !=
        0) {
      return ErrorHandling::Err(
          std::format("{}: failed to bind socket: {}",
                      ErrorHandling::format_source_location(),
                      ErrorHandling::get_error_message(errno)));
    }
    return {};
  ...
  template <typename Address>
  [[nodiscard]] auto connect(const Address &address) const
      -> ErrorHandling::Result<void, std::string> {
    if (::connect(m_socket_fd, (const ::sockaddr *)&address, sizeof(Address)) !=
        0) {
      return ErrorHandling::Err(
          std::format("{}: failed to connect socket: {}",
                      ErrorHandling::format_source_location(),
                      ErrorHandling::get_error_message(errno)));
    }
    return {};
  ...
  [[nodiscard]] auto listen(const int backlog_size) const
      -> ErrorHandling::Result<void, std::string> {
    if (::listen(m_socket_fd, backlog_size) != 0) {
      return ErrorHandling::Err(
          std::format("{}: failed to listen socket: {}",
                      ErrorHandling::format_source_location(),
                      ErrorHandling::get_error_message(errno)));
    }
    return {};
  ...
  template <typename T>
  [[nodiscard]] auto write(const T &value) const
      -> ErrorHandling::Result<void, std::string> {
    ...
    if (bytes_written != size) {
      return ErrorHandling::Err(
          std::format("{}: only wrote {} of {} bytes from socket: {}",
                      ErrorHandling::format_source_location(), bytes_written,
                      size, ErrorHandling::get_error_message(errno)));
    }
    return {};
  }
  ...
  [[nodiscard]] auto write(const std::string &data) const
      -> ErrorHandling::Result<std::size_t, std::string> {
  ...
  [[nodiscard]] auto write_exactly(const std::span<std::byte> &data) const
    ...
    if (*result != data.size()) {
      return ErrorHandling::Err(
          std::format("{}: only wrote {} of {} bytes to socket: {}",
                      ErrorHandling::format_source_location(), *result,
                      data.size(), ErrorHandling::get_error_message(errno)));
    }
    return {};
  }

  [[nodiscard]] auto write_exactly(const std::string &data) const -> auto {
    return write_exactly(
        std::span<std::byte>((std::byte *)data.data(), data.size()));
  }
  ...
  [[nodiscard]] auto read() const -> ErrorHandling::Result<T, std::string> {
    ...
    if (bytes_read != size) {
      return ErrorHandling::Err(
          std::format("{}: only read {} of {} bytes from socket: {}",
